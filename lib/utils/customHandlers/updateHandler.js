    ...
    return {
      data: subKey.length > 0 ? (0, _helpers.updateIn)(_objectSpread({}, data, {}, successData, _defineProperty({}, subKey[0], data[subKey[0]])), subKey, function (_Data) {
        return function () {
          ...
          if (!Array.isArray(_Data)) return updateData(_Data, (0, _nullCheck.default)(successData, ".".concat(subKey.join('.'))), updateCallback);else if (Array.isArray(id) && key && Array.isArray(_Data)) return _Data.reduce(function (acc) {
            ...
            return _data[key] === id ? function () {
              index = index + 1;
              return updateData(_data, values[_values ? index : curr[key]] || _data, updateCallback);
            }() : _data;
          });
        ...
        if (!Array.isArray(data)) return updateData(data, successData, updateCallback);else if (Array.isArray(id) && key) return data.reduce(function (acc) {
          var curr = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
          return id.includes(curr[key]) ? function () {
            index = index + 1;
            return acc.concat([updateData(curr, values[_values ? index : curr[key]] || curr, updateCallback)]);
