...
var COMMON_HANDLER = function COMMON_HANDLER(payload, data, state, type) {
  ...
  function () {
    ...
    if (task.response && !task.dontUpdateResponseData) checkKey$3(task.response, 'task { response  : { data }}', 'object');
    ...
    if (_handler) {
      checkKey$3(_handler.handler, "".concat(_handler.name, " handler with key name handler"), 'function');
