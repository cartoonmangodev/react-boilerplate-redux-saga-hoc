...
var useQuery = function useQuery() {
  ...
  if (name) checkKey$1(name, 'reducer name', 'string', 'valid string'); // const store = useStore();
...
var COMMON_HANDLER = function COMMON_HANDLER(payload, data) {
  ...
  function () {
    ...
    if (task.response && !task.dontUpdateResponseData) checkKey$2(task.response, 'task { response  : { data }}', 'object');
