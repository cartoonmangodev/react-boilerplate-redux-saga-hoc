...
const makeSelectAuthenticationState = _ref => {
  ...
  return () => reselect.createSelector(selectAuthenticationDomain(initialState, generatorKey), substate => newObject(Object.keys(InitialState).reduce((acc, key) => ({
    ...acc,
    [key]: substate[key]
  }), {}), Object.keys(apiEndPoints[generatorKey]).reduce((acc, key) => ({
    ...acc,
...
function loaderGenerator(_ref) {
  ...
  return function* () {
    yield effects.put(apiLoadingStatus({
      type,
      payload: commonData,
      status: false
    }));
...
const debounce = function (ms, pattern, task, isEvery) {
  ...
  return effects.fork(function* () {
    let taskID;
    while (true) {
      let action = yield effects.take(pattern);
      while (true) {
        const {
          debounced,
          latestAction
        ...
        if (debounced) {
          ...
          taskID = yield effects.fork(task, ...args, action);
          break;
        }
        action = latestAction;
      }
...
function _sagaHandler (_ref2) {
  ...
  function commonGenerator(_ref3) {
    ...
    return function* () {
      ...
      if (!__payload) {
        yield effects.call(loaderGenerator, {
          type,
          commonData: {
      ...
      while (loop) {
        ...
        if (!dontUpdateReducer) requestData = yield effects.call(requestResponseHandler, {
          type,
          action,
          request: request$1,
          payload: commonData,
        ...
        if (polling && callAfterDelay && loop && count === 1 && action && action.cancel) {
          ...
          if (cancelTask) {
            loop = false;
            if (!dontUpdateReducer) yield effects.call(requestResponseHandler, {
              type,
              action,
              payload: commonData,
              actionData: rest,
        ...
        try {
          ...
          if (cacheControl && request$1.method === 'GET' && _cache[cacheId] && !polling) {
            ...
            const {
              ...
              posts: typeof asyncFunction === 'function' ? effects.call(asyncFunction, ...(Array.isArray(pollingRequestConfig && pollingRequestConfig.asyncFunctionParams || asyncFunctionParams) ? pollingRequestConfig && pollingRequestConfig.asyncFunctionParams || asyncFunctionParams : [])) : effects.call(axios, {
                ...request$1,
                ...(pollingRequestConfig && pollingRequestConfig.axiosConfig || axiosConfig)
              }),
              cancel: effects.take(action.cancel),
              cancel_filter: effects.take(cancelKey && typeOf(cancelKey) === 'string' && cancelKey.length ? `${action.cancel}_[${cancelKey}]` : action.cancel)
            });
          ...
          if (data && postData.data) {
            ...
            let loader = null;
            if (!dontUpdateReducerOnSucess && !dontUpdateReducer) loader = yield effects.call(requestResponseHandler, {
              data,
              type,
              action,
              payload: commonData,
            ...
            if (loader) yield effects.call(loaderGenerator, {
              type,
              commonData
            });
            ...
            if (!customMethod && !dontUpdateReducer) yield effects.call(requestResponseHandler, {
              type,
              action,
              payload: commonData,
              actionData: rest,
            ...
            if (customMethod !== ON_UNMOUNT && !dontUpdateReducer) yield effects.call(loaderGenerator, {
              type,
              commonData
            });
            loop = false;
          } else if (process.env.NODE_ENV === 'test' && action.success) yield effects.put(action.success({
            data
          }));else {
            _success = {
              response: postData,
            ...
            if (!dontUpdateReducer) yield effects.call(loaderGenerator, {
              type,
              commonData
            });
          }
          ...
          try {
            ...
            if (!polling && retry && retry - 1 >= count) {
              ...
              if (AxiosDefault.isCancel(error) && action.cancel && !dontUpdateReducer && !dontUpdateReducerOnError) {
                ...
                yield effects.call(requestResponseHandler, {
                  type,
                  action,
                  payload: commonData,
                  actionData: rest,
                ...
                const loader = yield effects.call(requestResponseHandler, {
                  error: {
                    response: {
                      data: {
                        status: errorStatus,
    ...
    switch (method) {
      ...
      case ON_SUCCESS:
        if ([200, 201].includes(successStatus)) {
          yield effects.put(action.success({
            data: successData,
            ...restSuccessData
          }));
        } else return true;
      ...
      case ON_ERROR:
        {
          if (errorStatus) yield effects.put(action.error({
            data: errorData
          }));else yield effects.put(action.error({
            data: {}
          }));
          break;
        }
  ...
  return (
    ...
    function (_ref2) {
      ...
      return function* () {
        ...
        if (sagaFunction) {
          return yield effects.call(sagaFunction, newObject({
            type,
            constants,
        ...
        switch (method) {
          case ON_REQUEST:
            return requestData;
          default:
            return yield effects.call(DEFAULT_SAGA_HANDLER$1);
        }
        // switch (type) {
        //   case authenticationConstants.REGISTER_API[CALL]:
        //     switch (method) {
...
var Saga = (_ref => {
  ...
  // eslint-disable-next-line func-names
  const saga = function* () {
    yield effects.all(generatorPattern.concat(OtherGenerator || []));
  };
  return {
    saga,
    Generator
...
var index = (_ref => {
  ...
  return function () {
    ...
    const _useHocHook = () => {
      const isInjected = React.useRef(isMounted[reducerName]);
    ...
    const hoc = WrapperComponent => {
      ...
      const mapStateToProps = useHook || !_mapStateToProps ? null : reselect.createStructuredSelector({
      ...
      const withConnect = reactRedux.connect(useHook || !_mapStateToProps ? null : mapStateToProps, mapDispatchToProps(componentActions, componentData, reducerName));
      if (nextJS) {
        if (useType !== FOR_INTERNAL_USE_ONLY) {
          showDepricatedMessage(reducerName);
          return WithHoc;
...
function configureStore() {
  ...
  const sagaMiddleware = createSagaMiddleware__default(reduxSagaMonitorOptions);
  ...
  // eslint-disable-next-line no-underscore-dangle
  const _enhancers = [redux.applyMiddleware(...middlewares), ...enhancers];
  const store = redux.createStore(createReducer({}), initialState, composeEnhancers(..._enhancers));
...
const nextStore = _ref => {
  ...
  return function () {
    ...
    const enhancers = [redux.applyMiddleware(...middlewares), ..._enhancers];
    const store = redux.createStore(redux.combineReducers(reducer.reduce((acc, e) => ({
      ...acc,
      [e.name]: e.reducer
    ...
    if (req || !isServer) {
      store.sagaTask = sagaMiddleware.run(function* rootSaga() {
        yield effects.all(saga.map(effects.fork));
      });
    }
    // Make reducers hot reloadable, see http://mxs.is/googmo
    /* istanbul ignore next */
