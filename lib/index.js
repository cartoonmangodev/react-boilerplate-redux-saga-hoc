...
var RESET_DATA_HANDLER = 'Reset-Handler';
...
var convertData = function convertData(apiEndPoints) {
  return Object.keys(apiEndPoints).reduce(function (prev, curr) {
    var constants = Object.keys(apiEndPoints[curr]).reduce(function (acc, key) {
      ...
      return newObject(acc, _defineProperty({}, key, (_key = {}, _defineProperty(_key, CALL$1, "".concat(REDUCER_BASE_PATH).concat(curr, "/").concat(key, "_").concat(CALL$1)), _defineProperty(_key, SUCCESS, "".concat(REDUCER_BASE_PATH).concat(curr, "/").concat(key, "_").concat(SUCCESS)), _defineProperty(_key, CUSTOM, "".concat(REDUCER_BASE_PATH).concat(curr, "/").concat(key, "_").concat(CUSTOM)), _defineProperty(_key, ERROR, "".concat(REDUCER_BASE_PATH).concat(curr, "/").concat(key, "_").concat(ERROR)), _defineProperty(_key, CANCEL$2, "".concat(REDUCER_BASE_PATH).concat(curr, "/").concat(key, "_").concat(CANCEL$2)), _key)));
    }, {});
    ...
    var sagaConfig = Object.entries(apiEndPoints[curr]).reduce(function (acc, _ref) {
      ...
      return newObject(acc, _defineProperty({}, constants[key][CALL$1], {
        ...
        actions: actions[key],
        effect: value.effect === TAKE_EVERY && takeEvery$1
      }));
    }, {});
      ...
      data: function () {
        if (subKey.length > 0 && Array.isArray(getIn(oldData, subKey))) {
          ...
          return updateIn(_oldCopyData, subKey, function (_oldData) {
            ...
            return updateCallback ? updateCallback(_oldData, nullcheck(successData, ".".concat(subKey.join('.')), []), type, state) : isAppendTop ? nullcheck(successData, ".".concat(subKey.join('.')), []).concat(_oldData) : _oldData.concat(nullcheck(successData, ".".concat(subKey.join('.')), []));
          });
        }
        ...
        return updateCallback ? updateCallback(oldData, successData, type, state) : newData;
      }(),
      error: false,
      lastUpdated: generateTimeStamp(),
      statusCode: successDataStatusCode || statusCode,
...
var updateData = function updateData(data, successData, updateCallback, type, state) {
  ...
  if (updateCallback) return updateCallback(data, successData, type, state, config) || data;
...
var updateHandler = function updateHandler(_ref) {
  ...
  return function () {
    ...
    return {
      data: subKey.length > 0 ? updateIn(_objectSpread({}, data, {}, typeOf(successData) === 'object' && !dontUpdateResponseData ? successData : {}, _defineProperty({}, subKey[0], data[subKey[0]])), subKey, function (_Data) {
        return function () {
          var index = -1;
          ...
          if (!Array.isArray(_Data)) return updateData(_Data, nullcheck(successData, ".".concat(subKey.join('.'))), updateCallback, type, state);
          if (Array.isArray(id) && key && Array.isArray(_Data)) return _Data.reduce(function (acc) {
            ...
            return id.includes(curr[key]) ? function () {
              index += 1;
              return acc.concat([updateData(curr, values[_values ? index : curr[key]] || curr, updateCallback, type, state, {
                index: index,
          ...
          if ((id === 0 || id) && key) return _Data.map(function (_data) {
            return _data[key] === id ? function () {
              index += 1;
              return updateData(_data, values[_values ? index : _data[key]] || _data, updateCallback, type, state, {
                index: index,
                id: _values ? index : _data[key]
              });
            }() : _data;
          });
          return updateData(_Data, nullcheck(successData, ".".concat(subKey.join('.'))), updateCallback, type, state);
        }();
      }) : function () {
        var index = -1;
...
var executeTask$2 = function executeTask(_ref, _Data) {
  ...
  return updateCallback ? updateCallback(_updatedData, successData, type, state) || _Data : _updatedData;
};
    ...
    return isMutation ? _objectSpread({
      ...
      data: function () {
        if (subKey.length > 0) {
          ...
          return updateIn(_oldCopyData, subKey, function (_oldData) {
            if (clearData) return nullcheck(successData, ".".concat(subKey.join('.')));
            return updateCallback ? updateCallback(_oldData, nullcheck(successData, ".".concat(subKey.join('.'))), type, state) : _checkIsNotObject(nullcheck(successData, ".".concat(subKey.join('.')))) || _checkIsNotObject(nullcheck(_oldData, ".".concat(subKey.join('.')))) ? nullcheck(successData, ".".concat(subKey.join('.'))) : newObject(_oldData, nullcheck(successData, ".".concat(subKey.join('.'))));
          });
        }

        return updateCallback ? updateCallback(oldData, successData, type, state) : _checkIsNotObject(successData) || _checkIsNotObject(oldData) || clearData ? successData : newObject(oldData, successData);
      }(),
      statusCode: successDataStatusCode || statusCode,
      error: false,
      lastUpdated: generateTimeStamp(),
    ...
    return {
      data: function () {
        if (subKey.length > 0 && Array.isArray(getIn(oldData, subKey))) {
          ...
          return updateIn(_oldCopyData, subKey, function (_oldData) {
            if (clearData) return nullcheck(successData, ".".concat(subKey.join('.')), []);
            return updateCallback ? updateCallback(_oldData, nullcheck(successData, ".".concat(subKey.join('.')), []), type, state) : Array.isArray(_oldData) ? function () {
...
var callbackHandler = function callbackHandler(_ref) {
  ...
  var callback = _ref$task.callback,
      ...
      state = _ref.state,
      type = _ref.type,
      rest = _objectWithoutProperties$1(_ref, ["task", "successData", "successDataStatusCode", "state", "type"]);
...
var resetReducerHandler = function resetReducerHandler(_ref) {
  var _ref$task = _ref.task;
  _ref$task = _ref$task === void 0 ? {} : _ref$task;
  var callback = _ref$task.callback,
      _ref$successData = _ref.successData,
...
var checkKey$2 = function checkKey(key, name, dataType, message) {
...
var useQuery = function useQuery() {
  ...
  if (name) checkKey$2(name, 'reducer name', 'string', 'valid string'); // const store = useStore();
...
var useMutation = function useMutation(reducerName) {
  ...
  var _callback = React.useCallback(function (_ref23) {
    ...
    if (!type) checkKey$2(null, 'key', 'string', 'valid string');
    ...
    checkKey$2(filter, 'filter', 'array');
    checkKey$2(type, 'key', 'string');
...
var toPromiseAllFunction = function toPromiseAllFunction() {
  ...
  return function () {
    ...
    return Promise.all(actions.map(function (action, i) {
      return new Promise(function (resolve, reject) {
        return action(_objectSpread({}, (typeOf(config) === 'object' ? config : config[i] && config[i].config) || defaultConfig.config || {}, {
          resolve: resolve,
          reject: reject,
...
var COMMON_HANDLER = function COMMON_HANDLER(payload, data, state, type) {
  ...
  function () {
    ...
    if (task.response && !task.dontUpdateResponseData) checkKey$3(task.response, 'task { response  : { data }}', TYPE_OBJECT);
    ...
    if (_handler) {
      checkKey$3(_handler.handler, "".concat(_handler.name, " handler with key name handler"), TYPE_FUNCTION);
...
var DEFAULT_REDUCER_HANDLER = function DEFAULT_REDUCER_HANDLER(_ref3) {
  ...
  for (var i = 0; i < _method.length; i += 1) {
    switch (_method[i]) {
          ...
          if (_tasks && Array.isArray(_tasks) && _tasks.length > 0) {
              ...
              if (Array.isArray(_updateDataReducerKey) && _updateDataReducerKey.length > 0) {
                var _loop2 = function _loop2(l) {
                  DATA = newObject(DATA, function (_ref4) {
                    ...
                    return _defineProperty({}, _updateDataReducerKey[l] || type, _commonHandler(Data, state, _updateDataReducerKey[l] || type));
                  });
                };
                ...
                DATA = newObject(DATA, function (_ref6) {
                  var Data = _ref6[_updateDataReducerKey || type];
                  return _defineProperty({}, _updateDataReducerKey || type, _commonHandler(Data, state, _updateDataReducerKey || type));
                });
              }
            };
            ...
            var _loop3 = function _loop3(j) {
              DATA = newObject(DATA, function (_ref8) {
                ...
                return _defineProperty({}, updateDataReducerKey[j] || type, commonHandler(Data, state, updateDataReducerKey[j] || type));
              });
            };
      ...
      case ON_ERROR:
        {
          var _updatedState = newObject(DATA, function (_ref12) {
...
function _sagaHandler (_ref2) {
  ...
  function commonGenerator(_ref3) {
    var _ref3$payload, resolve, reject, isReject, _ref3$payload$dontUpd, dontUpdateReducer, _ref3$payload$request, _ref3$payload$request2, asyncFunction, _ref3$payload$request3, asyncFunctionParams, _ref3$payload$request4, payload, _ref3$payload$request5, params, query, delayFunction, _ref3$payload$request6, dontUpdateReducerOnSucess, _ref3$payload$request7, dontUpdateReducerOnError, requestAxios, _ref3$payload$request8, paramsSerializer, cancelKey, _ref3$payload$request9, axiosConfig, _ref3$payload$request10, cacheControl, _ref3$payload$request11, errorDataHandling, _ref3$payload$request12, clearDataOnError, _ref3$payload$request13, polling, _ref3$payload$request14, errorParser, _ref3$payload$request15, isResponseErrorParser, _ref3$payload$request16, Delay, _ref3$payload$request17, retry, _ref3$payload$request18, pollingCount, _ref3$payload$request19, callAfterDelay, rest, _ref3$payload$callbac, successCallback, errorCallback, logoutCallback, finalCallback, pollingCallback, cancelCallback, restCallback, restPayload, type, loop, count, pollingRequestConfig, POLLING_RESPONSE_DATA, _loop, _ret;
      ...
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            ...
            resolve = _ref3$payload.resolve, reject = _ref3$payload.reject, isReject = _ref3$payload.isReject, _ref3$payload$dontUpd = _ref3$payload.dontUpdateReducer, dontUpdateReducer = _ref3$payload$dontUpd === void 0 ? false : _ref3$payload$dontUpd, _ref3$payload$request = _ref3$payload.request;
            _ref3$payload$request = _ref3$payload$request === void 0 ? {} : _ref3$payload$request;
            _ref3$payload$request2 = _ref3$payload$request.asyncFunction, asyncFunction = _ref3$payload$request2 === void 0 ? null : _ref3$payload$request2, _ref3$payload$request3 = _ref3$payload$request.asyncFunctionParams, asyncFunctionParams = _ref3$payload$request3 === void 0 ? null : _ref3$payload$request3, _ref3$payload$request4 = _ref3$payload$request.payload, payload = _ref3$payload$request4 === void 0 ? {} : _ref3$payload$request4, _ref3$payload$request5 = _ref3$payload$request.params, params = _ref3$payload$request5 === void 0 ? {} : _ref3$payload$request5, query = _ref3$payload$request.query, delayFunction = _ref3$payload$request.delayFunction, _ref3$payload$request6 = _ref3$payload$request.dontUpdateReducerOnSucess, dontUpdateReducerOnSucess = _ref3$payload$request6 === void 0 ? false : _ref3$payload$request6, _ref3$payload$request7 = _ref3$payload$request.dontUpdateReducerOnError, dontUpdateReducerOnError = _ref3$payload$request7 === void 0 ? false : _ref3$payload$request7, requestAxios = _ref3$payload$request.axios, _ref3$payload$request8 = _ref3$payload$request.paramsSerializer, paramsSerializer = _ref3$payload$request8 === void 0 ? {
              arrayFormat: 'brackets'
            } : _ref3$payload$request8, cancelKey = _ref3$payload$request.cancelKey, _ref3$payload$request9 = _ref3$payload$request.axiosConfig, axiosConfig = _ref3$payload$request9 === void 0 ? {} : _ref3$payload$request9, _ref3$payload$request10 = _ref3$payload$request.useCache, cacheControl = _ref3$payload$request10 === void 0 ? false : _ref3$payload$request10, _ref3$payload$request11 = _ref3$payload$request.errorDataHandling, errorDataHandling = _ref3$payload$request11 === void 0 ? true : _ref3$payload$request11, _ref3$payload$request12 = _ref3$payload$request.clearDataOnError, clearDataOnError = _ref3$payload$request12 === void 0 ? false : _ref3$payload$request12, _ref3$payload$request13 = _ref3$payload$request.polling, polling = _ref3$payload$request13 === void 0 ? false : _ref3$payload$request13, _ref3$payload$request14 = _ref3$payload$request.errorParser, errorParser = _ref3$payload$request14 === void 0 ? false : _ref3$payload$request14, _ref3$payload$request15 = _ref3$payload$request.defaultErrorParser, isResponseErrorParser = _ref3$payload$request15 === void 0 ? false : _ref3$payload$request15, _ref3$payload$request16 = _ref3$payload$request.delay, Delay = _ref3$payload$request16 === void 0 ? 8000 : _ref3$payload$request16, _ref3$payload$request17 = _ref3$payload$request.retry, retry = _ref3$payload$request17 === void 0 ? 0 : _ref3$payload$request17, _ref3$payload$request18 = _ref3$payload$request.pollingCount, pollingCount = _ref3$payload$request18 === void 0 ? 'unlimited' : _ref3$payload$request18, _ref3$payload$request19 = _ref3$payload$request.callAfterDelay, callAfterDelay = _ref3$payload$request19 === void 0 ? false : _ref3$payload$request19, rest = _objectWithoutProperties$1(_ref3$payload$request, ["asyncFunction", "asyncFunctionParams", "payload", "params", "query", "delayFunction", "dontUpdateReducerOnSucess", "dontUpdateReducerOnError", "axios", "paramsSerializer", "cancelKey", "axiosConfig", "useCache", "errorDataHandling", "clearDataOnError", "polling", "errorParser", "defaultErrorParser", "delay", "retry", "pollingCount", "callAfterDelay"]), _ref3$payload$callbac = _ref3$payload.callback;
            _ref3$payload$callbac = _ref3$payload$callbac === void 0 ? {} : _ref3$payload$callbac;
            successCallback = _ref3$payload$callbac.successCallback, errorCallback = _ref3$payload$callbac.errorCallback, logoutCallback = _ref3$payload$callbac.logoutCallback, finalCallback = _ref3$payload$callbac.finalCallback, pollingCallback = _ref3$payload$callbac.pollingCallback, cancelCallback = _ref3$payload$callbac.cancelCallback, restCallback = _objectWithoutProperties$1(_ref3$payload$callbac, ["successCallback", "errorCallback", "logoutCallback", "finalCallback", "pollingCallback", "cancelCallback"]), restPayload = _objectWithoutProperties$1(_ref3$payload, ["resolve", "reject", "isReject", "dontUpdateReducer", "request", "callback"]), type = _ref3.type;
            loop = true;
            count = 1;
            pollingRequestConfig = {};
            POLLING_RESPONSE_DATA = {};
            _loop = /*#__PURE__*/_regeneratorRuntime.mark(function _loop() {
              var action, axios, CancelToken, source, url, commonData, actionBind, request$1, _query, _url, requestData, postData, cancelTask, _yield$race, _cancelTask, cacheId, _yield$race2, _postData, _cancelTask2, cancelFilterTask, data, statusKey, _ref4, _ref4$data, _ref4$data$status, successStatus, _ref4$data$message, successMessage, successCallbackResponse, _yield$race3, CancelPolling, _yield$race3$successC, _successCallbackResponse, loader, cancelResponse, _ref5, _ref5$response, customMethod, _ref6, _ref6$data, _ref6$data$status, _successStatus, _ref6$data$message, _successMessage, _ref7, _ref7$response, _ref7$response$data, _ref7$response$data2, errorData, _ref7$response$data$s, errorStatus, _ref7$response$data$m, errorMessage, errorCallbackResponse, _yield$race4, _CancelPolling2, _yield$race4$errorCal, _errorCallbackResponse, _loader, Cancelled, _yield$race5, _CancelPolling3, _yield$race6, _CancelPolling4, _yield$race7, _CancelPolling, pollingRes;
                ...
                while (1) {
                  switch (_context2.prev = _context2.next) {
                    ...
                    case 2:
                      ...
                      axios = action.api && action.api.axios || requestAxios || axiosInterceptors || request;
                    ...
                    case 33:
                      ...
                      checkKey$4(params, '{request: { params }}', 'object'); // throw new Error(
                    ...
                    case 38:
                      ...
                      if (pollingRequestConfig && pollingRequestConfig.payload) {
                        request$1.data = pollingRequestConfig && pollingRequestConfig.payload || payload; // eslint-disable-next-line no-loop-func
                    ...
                    case 61:
                      ...
                      loop = false; // const { response: { method: customMethod } = {} } = cancelTask || {};
                    ...
                    case 76:
                      ...
                      return race({
                        ...
                        cancel: take(action.cancel),
                        cancel_filter: take(cancelKey && typeOf(cancelKey) === 'string' && cancelKey.length ? "".concat(action.cancel, "_[").concat(cancelKey, "]") : action.cancel)
                      });

                    case 78:
                      _yield$race2 = _context2.sent;
                      _postData = _yield$race2.posts;
                      _cancelTask2 = _yield$race2.cancel;
                      cancelFilterTask = _yield$race2.cancel_filter;
                    ...
                    case 101:
                      _yield$race3 = _context2.sent;
                      CancelPolling = _yield$race3.cancel;
                      _yield$race3$successC = _yield$race3.successCallbackResponse;
                      _successCallbackResponse = _yield$race3$successC === void 0 ? null : _yield$race3$successC;
                      if (CancelPolling) loop = false;
                      successCallbackResponse = _successCallbackResponse; // successCallbackResponse = yield call(successCallback, {
                      //   response: postData,
                      //   posts: data,
                      //   data: data.data,
                      //   res: data && data.data && data.data.data,
                    ...
                    case 119:
                      if (!(cancelTask && (typeof source.cancel === 'function' || rest.onCancelTask))) {
                        _context2.next = 136;
                        break;
                      }

                      _context2.next = 122;
                      return (rest.onCancelTask || source.cancel)();

                    case 122:
                      cancelResponse = _context2.sent;
                    ...
                    case 141:
                      if (typeof successCallback === 'function' && !cancelTask) {
                        successCallback({
                          response: postData,
                          status: postData && postData.status // isError: true
                          // error: postData,
                          // response: postData,
                          // isError: true,
                    ...
                    case 150:
                      _context2.prev = 150;
                      _context2.t0 = _context2["catch"](70);
                      _context2.prev = 152;
                    ...
                    case 158:
                      if (isReject && reject && typeOf(reject) === 'function') reject({
                        status: 'ERROR',
                        error: _context2.t0 || 'NETWORK ERROR',
                      ...
                      _ref7$response$data2 = _ref7$response$data[action.api.errorDataKey || 'error'], errorData = _ref7$response$data2 === void 0 ? _context2.t0 && _context2.t0.response && _context2.t0.response.data || _context2.t0 && _context2.t0.response || '' : _ref7$response$data2, _ref7$response$data$s = _ref7$response$data.status, errorStatus = _ref7$response$data$s === void 0 ? _context2.t0 && _context2.t0.response && _context2.t0.response.data && (_context2.t0.response.data[action.api.errorStatusKey] || _context2.t0 && _context2.t0.response && _context2.t0.response.status) : _ref7$response$data$s, _ref7$response$data$m = _ref7$response$data.message, errorMessage = _ref7$response$data$m === void 0 ? _context2.t0 && _context2.t0.response && _context2.t0.response.data && _context2.t0.response.data[action.api.errorMessageKey] || _context2.t0 && _context2.t0.response && _context2.t0.response.statusText || _context2.t0 && _context2.t0.message || '' : _ref7$response$data$m;
                      ...
                      _context2.next = 170;
                      return race({
                        errorCallbackResponse: call(errorCallback, _objectSpread({
                          error: _context2.t0,
                          ...
                          isNetworkError: _context2.t0 && _context2.t0.request && _context2.t0.message === 'Network Error',
                          errorMessage: _context2.t0 && _context2.t0.message,
                          message: errorMessage,
                          status: errorStatus,
                          response: _context2.t0 && _context2.t0.response,
                          errors: errorData
                    ...
                    case 170:
                      _yield$race4 = _context2.sent;
                      _CancelPolling2 = _yield$race4.cancel;
                      _yield$race4$errorCal = _yield$race4.errorCallbackResponse;
                      _errorCallbackResponse = _yield$race4$errorCal === void 0 ? null : _yield$race4$errorCal;
                      if (_CancelPolling2) loop = false;
                      errorCallbackResponse = _errorCallbackResponse; // errorCallbackResponse = yield call(errorCallback, {
                      //   error,
                    ...
                    case 179:
                      if (!(AxiosDefault.isCancel(_context2.t0) && action.cancel && !dontUpdateReducer && !dontUpdateReducerOnError)) {
                        _context2.next = 186;
                        break;
                      }
                    ...
                    case 195:
                      _context2.prev = 195;
                      _context2.t1 = _context2["catch"](152);
                    ...
                    case 205:
                      _yield$race5 = _context2.sent;
                      _CancelPolling3 = _yield$race5.cancel;
                      if (_CancelPolling3) loop = false; // yield call(finalCallback, {
                      //   type,
                      //   action,
                      //   payload: commonData,
                      //   Cancelled,
                    ...
                    case 222:
                      _yield$race6 = _context2.sent;
                      _CancelPolling4 = _yield$race6.cancel;
                    ...
                    case 231:
                      _yield$race7 = _context2.sent;
                      _CancelPolling = _yield$race7.cancel;
                      pollingRes = _yield$race7.pollingRes;
              ...
              }, _loop, null, [[70, 150, 198, 217], [152, 195]]);
            });
...
var checkKey$5 = function checkKey(key, name, dataType) {
...
var index = (function (_ref) {
  var _ref$handlers = _ref.handlers,
      ...
      _mapStateToProps = _ref$mapStateToProps === void 0 ? true : _ref$mapStateToProps,
      ...
      _ref$hookWithHoc = _ref.hookWithHoc,
      hookWithHoc = _ref$hookWithHoc === void 0 ? false : _ref$hookWithHoc;
  ...
  return function () {
    ...
    var nextStateProps = null;
    var stateProps = null;
    invariant(!!reducerName && typeOf(reducerName) === 'string', '(react-boilerplate-redux-saga-hoc)  Expected `name` to be a non empty string');
    checkKey$5(apiEndPoints, 'apiEndPoints', 'object');
    checkKey$5(initialState, 'initialState', 'object');
    checkKey$5(dontResetOnLogout, 'dontReset', ['object', 'array']);
    if (sagaFunction) checkKey$5(sagaFunction, 'saga', 'function');
    checkKey$5(constantSaga, 'constantSaga', 'array');
    checkKey$5(handlers, 'handlers', 'array');
    if (constantReducer) checkKey$5(constantReducer, 'constantReducer', 'function');
    if (reducerFunction) checkKey$5(reducerFunction, 'reducer', 'function');
    if (createReducer) checkKey$5(createReducer, 'createReducer', 'function');
    ...
    var _useHocHook = function _useHocHook() {
      var inject = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
      var isInjected = React.useRef(false);

      if (!isMounted[reducerName] || isInjected.current || inject) {
        if (!isMounted[reducerName]) console.log("===== Successfully Injected Reducer - ".concat(reducerName, " ====="));
        if (!isInjected.current && !inject) isInjected.current = true;
        isMounted[reducerName] = true; // eslint-disable-next-line react-hooks/rules-of-hooks

        useInjectSaga(injectSagaConfig); // eslint-disable-next-line react-hooks/rules-of-hooks

        useInjectReducer(injectReducerConfig, createReducer);
      }

      var dispatch = reactRedux.useDispatch();
      if (!stateProps) stateProps = dispatch ? _objectSpread({}, componentData["".concat(reducerName, "_hoc")], {
        actions: redux.bindActionCreators(componentActions, dispatch),
        dispatch: dispatch
      }) : null;

      var _useState = React.useState(stateProps),
          _useState2 = _slicedToArray(_useState, 1),
          state = _useState2[0];
    ...
    var hoc = function hoc(WrapperComponent) {
      function WithHoc(props) {
        return /*#__PURE__*/React__default.createElement(WrapperComponent, _extends$3({}, commonProps, props));
      }
              ...
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    _context.next = 5;
                    return WrapperComponent.getInitialProps(_objectSpread({}, props, {}, mapDispatchToProps(componentActions, componentData, reducerName // eslint-disable-next-line prettier/prettier
                    )(store.dispatch)));
    ...
    var _useHocHookNextJs = function _useHocHookNextJs() {
      var inject = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
      var isInjected = React.useRef(false);
      ...
      var dispatch = reactRedux.useDispatch();
      if (!nextStateProps && dispatch) nextStateProps = _objectSpread({}, componentData["".concat(reducerName, "_hoc")], {
        actions: redux.bindActionCreators(componentActions, dispatch),
        dispatch: dispatch
      });

      var _useState3 = React.useState(nextStateProps),
...
var regex = {
  ...
  number: numberReg,
  float: decimalReg,
  postiveIntegerReg: postiveIntegerReg
};
...
exports.FormValidatorRegex = regex;
exports.HOC = index;
